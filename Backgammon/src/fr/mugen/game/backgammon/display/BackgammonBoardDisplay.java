...
public class BackgammonBoardDisplay implements Display {
  ...
  public BackgammonBoardDisplay(final Pane root) throws URISyntaxException {
    ...
    this.blackCheckerImage = new Image(BackgammonBoardDisplay.BLACK_CHECKER_IMG_PATH);
    this.whiteCheckerImage = new Image(BackgammonBoardDisplay.WHITE_CHECKER_IMG_PATH);
    this.cursorImageView = new ImageView(new Image(BackgammonBoardDisplay.CURSOR_IMG_PATH));
    this.cursorSelectedImageView = new ImageView(new Image(BackgammonBoardDisplay.SELECTION_CURSOR_IMG_PATH));

    this.diceSprite = new DiceSprite();

    this.diceRollingSound = new Media(
        getClass().getClassLoader().getResource(BackgammonBoardDisplay.ROLLING_DICE_SOUND_PATH).toURI().toString());
  }
  ...
  public void showHideColumnNumbers() {
    if (this.columnNumbersTexts != null) {
      System.out.println("Remove all");
      this.root.getChildren().removeAll(this.columnNumbersTexts);
      this.columnNumbersTexts = null;
      return;
    }

    System.out.println("Show");
    this.columnNumbersTexts = new ArrayList<Text>();
    for (final BackgammonColumn column : ((BackgammonBoard) this.game.getBoard()).getColumns()) {
      ...
      this.columnNumbersTexts.add(text);
      this.root.getChildren().add(text);
    }
  }
      ...
      public void run() {
        ...
        try {
          if (!BackgammonBoard.IS_CEMETERY(from)) {
            Thread.sleep(BackgammonBoardDisplay.COMPUTER_MOVE_TIME_INTERVAL);
