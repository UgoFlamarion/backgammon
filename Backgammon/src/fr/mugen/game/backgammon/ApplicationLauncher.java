...
public class ApplicationLauncher extends Application {
  ...
  private enum MenuElement {
	  EASY_AI("Easy AI", (root) -> {
		  JavaFXDisplay display = new JavaFXDisplay(root);
		  final Controls controls = new JavaFXControls(display);
		  
		  initGame(display, Arrays.asList(new HumanPlayer(controls, Color.WHITE),
			        ComputerPlayerFactory.createComputerPlayer(Color.BLACK, Difficulty.EASY)));
	  }, 1, 0),
	  ...
	  HARD_AI("Hard AI", (root) -> {
		  ...
		  initGame(display, Arrays.asList(new HumanPlayer(controls, Color.WHITE),
			        ComputerPlayerFactory.createComputerPlayer(Color.BLACK, Difficulty.HARD)));
	  }, 1, 2),
	  HUMAN_VS_HUMAN("Human VS Human", (root) -> {
		  JavaFXDisplay display = new JavaFXDisplay(root);
		  final Controls controls = new JavaFXControls(display);
		  
		  initGame(display, Arrays.asList(new HumanPlayer(controls, Color.WHITE), new HumanPlayer(controls, Color.BLACK)));
	  }, 0, 0),
	  ...
	  private MenuElement(String menuLabel, MenuCallable gameInitializer, int menuLevel, int position) {
		this.gameInitializer = gameInitializer;
		this.menuLevel = menuLevel;
		this.position = position;
		
		this.text = new Text(menuLabel);
		text.getStyleClass().add("menuElement");
	  }

	  public static MenuElement getNextMenuElement(MenuElement menuElement) {
		  for (MenuElement _menuElement : MenuElement.values())
			  if (_menuElement.menuLevel == menuElement.menuLevel
			   && _menuElement.position == menuElement.position + 1)
				  return _menuElement;
		  return null;
	  }
	  
	  public static MenuElement getPreviousMenuElement(MenuElement menuElement) {
		  for (MenuElement _menuElement : MenuElement.values())
			  if (_menuElement.menuLevel == menuElement.menuLevel
			   && _menuElement.position == menuElement.position - 1)
				  return _menuElement;
  ...
  private void selectMenu(MenuElement menuElement) {
	  if (menuElement == null)
		  return;
  ...
  public static void initGame(final Display display, final List<Player> players) throws InconsistentGameException {
	BackgammonGame game = new BackgammonGame(new BackgammonBoard(), players, new BackgammonRules(), display);
	game.start();
  }
