  ...
  public BackgammonMove findBestMove(BackgammonBoard board, BackgammonRules rules, HashMap<BackgammonColumn, List<BackgammonColumn>> possibilities) {
	  ...
	  possibilities.forEach((selectableColumn, columns) -> {
		  columns.forEach(column -> {
			  ...
			  Priority priority = new Priority(move, 0);
			  ...
			  if (!existEnemyCheckerBefore && move.getTo().getNumber() == 1 && move.getTo().getColor() == color)
					  priority.increaseWeight(Priority.priorityLevelToWeight(PROTECT_CHECKER_AND_CREATE_WALL_PRIORITY_LEVEL));
			  
			  if (!existEnemyCheckerBefore && move.getFrom().getNumber() == 1 && move.getTo().getNumber() > 0)
				  priority.increaseWeight(Priority.priorityLevelToWeight(PROTECT_CHECKER_PRIORITY_LEVEL));
			  
			  if (move.isEating())
				  priority.increaseWeight(Priority.priorityLevelToWeight(EAT_CHECKER_PRIORITY_LEVEL));
			  ...
			  if (((move.getTo().getNumber() == 0 && nextPossibilities.keySet().stream().filter(nextSelectableColumn -> {
				return nextPossibilities.get(nextSelectableColumn).stream().filter(nextColumn -> {
					return nextColumn.getPosition() == column.getPosition();
					  ...
					  || (moveCopy.getFrom().getNumber() == 1 && nextPossibilities.keySet().stream().filter(nextSelectableColumn -> {
							return nextPossibilities.get(nextSelectableColumn).stream().filter(nextColumn -> {
								return nextColumn.getPosition() == move.getTo().getPosition();
							}).count() > 0;
						  }).count() > 0)) && boardCopy.getDice().getRange() == 0)
				  priority.increaseWeight(Priority.priorityLevelToWeight(CAN_BE_PROTECTED_PRIORITY_LEVEL));
			  
			  if (BackgammonBoard.POSITION_TO_SIDE_COLOR(column.getPosition()) != this.color)
				  priority.increaseWeight(Priority.priorityLevelToWeight(NOT_IN_BASE_CAMP_PRIORITY_LEVEL));
...
//		return color != Color.WHITE ? sameWeightPriorities.stream().max(comparator).get().getMove() : sameWeightPriorities.stream().min(comparator).get().getMove();
        Comparator<? super Priority> comparator = (p1, p2) -> p2.getMove().getFrom().getPosition() - p1.getMove().getFrom().getPosition();
        return (color == Color.WHITE && isProtectCheckerPrioritylevel) || (color == Color.BLACK && !isProtectCheckerPrioritylevel) ? sameWeightPriorities.stream().max(comparator).get().getMove() : sameWeightPriorities.stream().min(comparator).get().getMove();
	  }
...
//	  final Priority topPriority = new Priority(new BackgammonMove(new BackgammonColumn(17), new BackgammonColumn(19)), 100);
//	  
//	  List<Priority> priorities = new ArrayList<>();
//	  priorities.add(new Priority(new BackgammonMove(new BackgammonColumn(17), new BackgammonColumn(19)), 100));
//	  priorities.add(new Priority(new BackgammonMove(new BackgammonColumn(15), new BackgammonColumn(19)), 100));
//	  
//	  List<Priority> sameWeightPriorities = priorities.stream().filter(priority -> priority != topPriority && priority.getWeight() == topPriority.getWeight()).collect(Collectors.toList());
//	  if (sameWeightPriorities.size() > 0) {
//		System.out.println("Multiple priorities with same weight found.");
...
//        System.out.println("color : " + color);
//        Comparator<? super Priority> comparator = (p1, p2) -> p2.getMove().getFrom().getPosition() - p1.getMove().getFrom().getPosition();
//		BackgammonMove move = (color == Color.WHITE && isProtectCheckerPrioritylevel) || (color == Color.BLACK && !isProtectCheckerPrioritylevel) ? sameWeightPriorities.stream().max(comparator).get().getMove() : sameWeightPriorities.stream().min(comparator).get().getMove();
//		
//		System.out.println(move.getFrom().getPosition());
//	  }
	  BackgammonBoard board = new BackgammonBoard();
	  BackgammonColumn column = new BackgammonColumn(17);
	  boolean existEnemyCheckerBefore = board.getColumns().stream().filter(c -> {
		  boolean colorTest = c.getColor() != color;
		  boolean posTest = c.getNumber() > 0 && (c.getPosition() - column.getPosition()) * (color == Color.WHITE ? -1 : 1) > 0;
