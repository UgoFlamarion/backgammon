...
//    System.out.println("FROM " + from+ " TO " + to);
	...
	return from.getColor() == playerColor
		   && from.getNumber() > 0
		   && (Color.NONE == to.getColor() 
		   	   || from.getColor() == to.getColor() 
		   	   || to.getNumber() == 1) // Eat it !
...
//      return false;
//    if (Color.WHITE.equals(from.getColor()) && !playerIsWhite || !(from.isWhite() && playerIsWhite))
//      return false;
//
//    final int moveLength = from.getPosition() - to.getPosition();
//    if (Color.WHITE.equals(playerColor) && moveLength <= 0 || !playerIsWhite && moveLength >= 0)
//      return false;
//
...
//    return true;
  ...
  public boolean isSelectable(final BackgammonBoard board, final BackgammonPlayer player, final BackgammonColumn column) {
	  return column.getColor() == player.getColor() 
  			&& column.getNumber() > 0; 
  }
  ...
  public int getNextPossiblePositionOnLeft(final Board board, final Player player, final BackgammonColumn currentColumn, final BackgammonColumn selectedColumn) {
    ...
    final Stream<BackgammonColumn> columns = ((BackgammonBoard) board).getColumns().stream()
        .filter(column -> {
        	return  (column.getPosition() - currentPosition) * sideFactor < 0
        			&& column.getPosition() / 13 == currentPosition / 13 // Avoid going to next line
  ...
  public int getNextPossiblePositionOnRight(final Board board, final Player player, final BackgammonColumn currentColumn, final BackgammonColumn selectedColumn) {
	  final int currentPosition = currentColumn.getPosition();
	  final int sideFactor = (currentPosition <= 12 ? 1 : -1);
	    final Stream<BackgammonColumn> columns = ((BackgammonBoard) board).getColumns().stream()
	        .filter(column -> {
  ...
  public int getNextPossiblePositionUpward(final Board board, final Player player, final BackgammonColumn currentColumn, final BackgammonColumn selectedColumn) {
  ...
  public int getNextPossiblePositionDownward(final Board board, final Player player, final BackgammonColumn currentColumn, final BackgammonColumn selectedColumn) {
	  int currentPosition = currentColumn.getPosition();
